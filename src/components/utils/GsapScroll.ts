...
export function setCharTimeline(
  ...
  character?.children.forEach((object: THREE.Object3D) => {
    ...
    if (object.name === "screenlight") {
      const screenMesh = object as MeshWithMaterial;
  ...
  if (window.innerWidth > 1024) {
    if (character) {
      ...
      if (monitor) {
        const monitorMesh = monitor as THREE.Object3D & { material: THREE.Material; position: THREE.Vector3 };
        tl2.to(
      ...
      if (screenLight) {
        const screenMesh = screenLight as THREE.Object3D & { material: THREE.Material };
        tl2.to(
          screenMesh.material,
